...
async def test_extraction_goes_to_the_nano_model_deterministically_in_json_mode(
    akili_paths, stub_embeddings, fake_llm
):
    """
    ...
    await _extract(json.dumps(GOOD), fake_llm)

    assert fake_llm.main_calls == []
    [call] = fake_llm.extraction_calls
...
async def test_an_unreachable_extractor_is_reported_not_raised(
    ...
    landed, problems = await _extract(ConnectionError("endpoint down"), fake_llm)
    assert landed == {}
    assert any("could not be reached" in p for p in problems), problems


async def test_an_empty_completion_is_reported(akili_paths, stub_embeddings, fake_llm):
    """message.content can be None; it must not be stored as the string 'None'."""
    landed, problems = await _extract(None, fake_llm)
    assert landed == {}
...
async def test_bare_json_object_parses(akili_paths, stub_embeddings, fake_llm):
    landed, errors = await _extract(json.dumps(GOOD), fake_llm)
    assert landed == {
...
async def test_fenced_json_parses(akili_paths, stub_embeddings, fake_llm):
    payload = "```json\n" + json.dumps(GOOD) + "\n```"
    landed, errors = await _extract(payload, fake_llm)
...
async def test_trailing_prose_after_the_object_is_tolerated(
    ...
    landed, problems = await _extract(payload, fake_llm)
...
async def test_a_fence_inside_prose_is_tolerated(akili_paths, stub_embeddings, fake_llm):
