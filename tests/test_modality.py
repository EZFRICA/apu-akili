...
def test_text_and_braille_get_a_source_list_at_the_end(mode):
    rendered = render_answer("Photosynthesis produces glucose.", SOURCES, mode)
    assert rendered.spoken is None
    assert rendered.written.startswith("Photosynthesis produces glucose.")
...
def test_voice_names_sources_aloud_without_urls():
    rendered = render_answer("Photosynthesis produces glucose.", SOURCES, InteractionMode("voice", "voice"))
    assert rendered.spoken == "According to Wikipedia and Lumni, Photosynthesis produces glucose."
...
def test_voice_with_a_text_display_also_gets_the_written_list():
    rendered = render_answer(
        "Photosynthesis produces glucose.", SOURCES,
...
def test_text_input_with_voice_output_gets_both():
    rendered = render_answer("Answer.", SOURCES, InteractionMode("text", "voice"))
    assert rendered.spoken.startswith("According to")
    assert "Sources:" in rendered.written


def test_without_sources_the_answer_is_unchanged():
    assert render_answer("Answer.", [], InteractionMode("text", "text")).written == "Answer."
    assert render_answer("Answer.", [], InteractionMode("voice", "voice")).spoken == "Answer."


def test_unknown_sites_are_named_after_their_domain():
    assert spoken_attribution([Source("x", "https://www.example-ecole.org/page")]) == "According to Example-ecole"
