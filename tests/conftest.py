...
# Every provider needs a key before a client can be built. Fake, non-empty ones let the
# router build; the clients themselves are replaced by `fake_llm` before any call, and
# sockets are blocked in the tests that reach inference, so the values never leave.
for _provider_key in ("NEBIUS_API_KEY", "GEMINI_API_KEY", "NVIDIA_API_KEY"):
    os.environ.setdefault(_provider_key, "test-key-not-real-do-not-use")
...
class FakeLLMClient:
    """
    Stands in for the OpenAI client behind every provider in apu.inference.llm.

    Replaces the router's clients rather than call_main_model / call_extraction_model
    themselves, so the router's own code still runs and the tests verify the real routing:
    which model id each role's call is sent to, and with which parameters.
    ...
    def _create(self, model, messages, **kwargs):
        ...
        elif model in (config.EXTRACTION_MODEL, config.QUERY_GATE_MODEL):
            # One queue for the small models: a turn that searches asks the query gate and
            # then the write-back, in that order.
            queue = self.extraction_replies
        else:
            raise AssertionError(f"unexpected model id sent for inference: {model!r}")
...
def fake_llm(monkeypatch):
    ...
    fake = FakeLLMClient()
    monkeypatch.setattr(llm, "_clients", {provider: fake for provider in llm.PROVIDERS})
    return fake
...
# The search-query gate (apu.guardrails.classifier.classify_search_query) runs on the
# extraction model in JSON mode, so a turn that searches consumes these before the memory
# write-back's own reply.
SCHOOL_QUERY_VERDICT = '{"verdict": "SCHOOL"}'
OFF_TOPIC_QUERY_VERDICT = '{"verdict": "OFF_TOPIC"}'
TEST_SESSION_ID = "session-test"
TEST_CLASS_ID = "lycee-cocody:3eA"


def scripted_verdict(prompt) -> str:
    if WELFARE_MARKER in str(prompt):
        return "WELFARE"
