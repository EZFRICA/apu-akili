"""
The student notebook: saving (button and chat), the three kinds of entry, braille sheets,
and the notebook staying out of the tutor's context.

Target: apu/notebook/, apu/tools/notebook.py, apu/modality/braille/sheet.py,
...
async def test_a_full_answer_is_kept_as_written_without_a_model_call(store, fake_llm):
...
async def test_key_points_are_condensed_in_plain_text(store, fake_llm, no_network):
    """On the small write-back model: 0.81 s against 6.89 s on the tutor model (docs/models.md)."""
    fake_llm.extraction_replies = [f"  {KEY_POINTS}\n"]
    saved = await service.save_entry(student_id="eleve-aya", class_level="6eme", subject="math",
...
async def test_an_excerpt_keeps_the_chosen_passage(store, fake_llm):
    saved = await service.save_entry(student_id="eleve-aya", class_level="6eme", subject="math",
                                     kind="excerpt", answer=PREVIOUS_ANSWER, excerpt="3/12 + 2/12 = 5/12",
                                     origin="button", store=store)
    assert saved.text == "3/12 + 2/12 = 5/12" and fake_llm.calls == []
...
async def test_an_empty_condensation_is_retried_once_then_reported(store, fake_llm, no_network):
    fake_llm.extraction_replies = [None]
    with pytest.raises(service.NotebookUnavailable):
        await service.save_entry(student_id="eleve-aya", class_level="6eme", subject="math",
                                 kind="key_points", answer=PREVIOUS_ANSWER, origin="button", store=store)
    assert len(fake_llm.calls) == 2 and store.entries("eleve-aya") == []
...
async def test_the_summary_is_written_by_nemotron_super_from_the_selected_entries(fake_llm, no_network):
    ...
    prompt = call["messages"][0]["content"]
    assert "note one" in prompt and "note two" in prompt and "braille" in prompt
    with pytest.raises(ValueError, match="nothing"):
        await service.summarize_entries([])


async def test_a_sheet_cannot_send_the_whole_notebook_to_the_model(fake_llm, monkeypatch, no_network):
    ...
    assert fake_llm.calls == [], "nothing was sent"


def test_a_braille_sheet_has_unicode_braille_and_an_embosser_file(liblouis):
    from apu.modality.braille.sheet import braille_sheet

    sheet = braille_sheet("**Fractions**\n1/4 + 1/6 = 5/12")
    assert sheet.text == "Fractions\n1/4 + 1/6 = 5/12"
    assert all(0x2800 <= ord(c) <= 0x28FF or c in " \n" for c in sheet.unicode_braille)
    assert sheet.brf and sheet.pages == 1
...
@pytest.fixture
def turn(akili_paths, no_network, stub_embeddings, fake_llm):
    return fake_llm


async def test_asking_the_tutor_saves_the_key_points_of_its_previous_answer(turn):
    import apu.runtime.agent as agent
    # The tutor asks for the tool then confirms; the key points come from the small model.
...
async def test_a_failed_condensation_is_reported_and_the_tutor_says_so(turn):
    ...
    turn.extraction_replies = [ConnectionError("provider down"), "{}"]
    ...
    assert any("provider down" in problem for problem in out["tool_problems"])
...
async def test_a_save_needs_the_sessions_current_validated_turn(akili_paths, fake_llm):
...
async def test_notebook_tool_alias_and_live_intents_helpers(store):
    ...
    assert notebook_tool.append_to_notebook is append_to_notebook
    assert notebook_tool.read_notebook is read_notebook

    # Test append_to_notebook and read_notebook
    res = append_to_notebook("eleve-aya", content="Remember: 1/2 = 2/4", title="Fractions Rule", store=store)
    assert res["entry_id"]
    assert res["student_id"] == "eleve-aya"
    assert res["content"] == "Remember: 1/2 = 2/4"

    read_res = read_notebook("eleve-aya", store=store)
    assert len(read_res["entries"]) == 1
    assert read_res["entries"][0]["content"] == "Remember: 1/2 = 2/4"

    # Test live voice handle_save_notebook
    history = [
        {"role": "user", "content": "Explain fractions"},
        {"role": "assistant", "content": "A fraction represents equal parts of a whole."},
    ]
    save_result = handle_save_notebook("eleve-aya", history, "save that to my notebook")
    assert "fraction represents" in save_result["content"]
