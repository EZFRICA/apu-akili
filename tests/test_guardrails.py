...
from tests.conftest import OFF_TOPIC_MARKER, WELFARE_MARKER, make_classifier_llm
...
def test_the_guard_runs_the_configured_model_through_an_openai_compatible_endpoint():
    """
    The Colang config is a file NeMo reads on its own, so it can drift from apu.config, which
    the interface and the router read. This pins them together.
    """
    ...
    base_url, key_variable = PROVIDERS[config.GUARD_PROVIDER]

    assert main.engine == "openai", "an OpenAI-compatible client, not OpenAI itself"
    assert main.model == config.GUARD_MODEL
    assert main.api_key_env_var == key_variable
    assert main.parameters["base_url"] == base_url

    llm = LLMRails(rails_config).llm   # built, never called: no network
    assert llm.provider_url.startswith(base_url.rstrip("/"))
...
# ── a student in distress is not a student misbehaving ──────────────────────

async def test_a_disclosure_is_answered_with_care_and_never_counted(env):
    """
    Measured before this path existed (docs/security.md): "my father hits me" and "I do not
    want to live any more" were classified off-topic, answered with the firm refusal, counted
    as attempts, and the third one wrote the child's words into the discipline record.
    """
    from apu.guardrails.actions import WELFARE_REPLY

    session = env.open()
    decision = await env.guard.check(session.session_id, f"{WELFARE_MARKER} My father hits me.")

    assert decision.outcome is TurnOutcome.WELFARE
    assert not decision.allowed, "the tutor does not answer this as schoolwork"
    assert decision.validated_turn is None, "and it cannot search the web about it"
    assert decision.reply.startswith(WELFARE_REPLY[:40])
    assert "trust" in decision.reply, "it points the child to a person"
    assert session.off_topic_count == 0, "a disclosure is not an off-topic attempt"
    assert env.events() == [], "and it is never written into the discipline record"


async def test_repeated_disclosures_never_escalate_as_misbehaviour(env):
    session = env.open()
    for _ in range(4):   # well past the class threshold of 2
        await env.guard.check(session.session_id, f"{WELFARE_MARKER} I am being bullied.")
    assert session.off_topic_count == 0 and env.events() == []


async def test_the_school_can_add_its_own_contact(env, monkeypatch):
    from apu import config

    monkeypatch.setattr(config, "WELFARE_CONTACT_TEXT", "Call the school counsellor on 1234.")
    session = env.open()
    decision = await env.guard.check(session.session_id, f"{WELFARE_MARKER} I feel unsafe.")
    assert decision.reply.endswith("Call the school counsellor on 1234.")
...
@pytest.mark.parametrize("raw,expected", [
    ...
    ("WELFARE", TurnOutcome.WELFARE),
    ("welfare", TurnOutcome.WELFARE),
    # A disclosure dressed as something else: welfare wins, because the costly mistake is
    # treating a child's disclosure as a topic violation.
    ("OFF_TOPIC WELFARE", TurnOutcome.WELFARE),
    ("<think>school or welfare?</think>\nWELFARE", TurnOutcome.WELFARE),
