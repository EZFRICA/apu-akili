...
def test_grade_1_english_produces_unicode_braille(liblouis):
    out = BrailleTranslator(BrailleGrade.GRADE_1).translate("hello class")
...
def test_grade_2_contracts_where_grade_1_does_not(liblouis):
    sentence = "the children in the class are working through the knowledge of fractions"
...
def test_line_breaks_are_kept(liblouis):
    out = BrailleTranslator().translate("one\n\ntwo")
    assert out.count("\n") == 2
...
def test_accented_and_long_text_is_fully_translated(liblouis):
    ...
    out = BrailleTranslator(language="fr").translate(text.strip())
    assert _is_unicode_braille(out)
    assert out.count("⠀") + out.count(" ") >= 100, "every word boundary survived"


def test_french_tables_are_available(liblouis):
    assert _is_unicode_braille(BrailleTranslator(BrailleGrade.GRADE_2, language="fr").translate("fractions"))
...
def test_the_embosser_refuses_text_that_is_not_braille_ascii():
    with pytest.raises(EmbosserError, match="Not Braille ASCII"):
        SimulatedEmbosser().emboss("hello")   # lowercase: print text, not Braille ASCII
...
def test_translation_to_embosser_end_to_end(liblouis, tmp_path):
    ascii_braille = BrailleTranslator(BrailleGrade.GRADE_2, BrailleEncoding.EMBOSSER).translate(
        "Fractions are used to share things out.\nSources:\n1. Wikipedia"
    )
