...
def test_importing_the_agent_without_a_nebius_key_succeeds():
    ...
    proc = _run(
        ...
        """,
        {"NEBIUS_API_KEY": "", "GEMINI_API_KEY": ""},
    )
    assert "IMPORT_OK" in proc.stdout, proc.stdout + proc.stderr


def test_a_role_fails_loudly_on_first_use_without_its_provider_key():
    """
    Importing the router is free; a missing key is reported when a role is actually called,
    and it names the variable to set. Roles whose provider has a key keep working.
    """
    proc = _run(
        """
        import apu.inference.llm as llm
        print("IMPORT_OK")
        try:
            llm.call_main_model([{"role": "user", "content": "hi"}])
        except llm.ProviderKeyMissing as e:
            print("RAISED:", e)
        """,
        {"GEMINI_API_KEY": "", "NEBIUS_API_KEY": "", "NVIDIA_API_KEY": ""},
    )
    assert "IMPORT_OK" in proc.stdout, proc.stdout + proc.stderr
...
def test_only_the_speech_path_may_use_another_provider(akili_paths):
    ...
    forbidden = re.compile(
        ...
        r"google\.generativeai|google\.genai|google\s+import\s+genai)\b",
        re.M,
    )
    offenders = [
...
async def test_a_turn_sends_the_answer_to_super_and_the_extraction_to_nano(
    akili_paths, no_network, stub_embeddings, fake_llm
):
    import apu.runtime.agent as agent

    fake_llm.main_replies = ["That makes 23/20."]
    fake_llm.extraction_replies = ['{"current_session": "Fractions in 6eme."}']
...
async def test_the_write_back_is_still_inline(
    ...
    fake_llm.main_replies = ["ok"]
    fake_llm.extraction_replies = ['{"current_session": "Fractions in 6eme."}']

    await agent.planner_node(_state())

    assert len(fake_llm.extraction_calls) == 1
...
async def test_the_tutor_is_told_to_answer_in_the_students_language(
    akili_paths, no_network, stub_embeddings, fake_llm
):
    """A model that follows its sources answered an English question in French (docs/models.md)."""
    import apu.runtime.agent as agent
    ...
    instructions = fake_llm.main_calls[0]["messages"][0]["content"]
    assert "LANGUAGE:" in instructions and "language the student wrote in" in instructions
...
async def test_a_turn_runs_end_to_end_through_the_graph(
    ...
    assert "What is 3/4 + 2/5?" in fake_llm.extraction_calls[0]["messages"][0]["content"]
...
async def test_a_failing_guard_means_no_answer(
    akili_paths, no_network, stub_embeddings, fake_llm, monkeypatch
):
    ...
    fake_llm.main_replies = ["should never be used"]
