...
@pytest.fixture
def turn(akili_paths, no_network, stub_embeddings, fake_llm, fake_tavily):
    return fake_llm, fake_tavily
...
async def test_an_off_topic_query_is_refused_even_on_a_validated_turn(turn):
    """
    The second gate, found necessary by the red team pass (docs/security.md): a real lesson
...
async def test_a_query_gate_that_cannot_answer_refuses_the_search(turn):
    ...
    assert out["refused_searches"] == ["fractions"]
...
async def test_a_spoken_answer_names_sources_without_urls(turn):
    ...
    assert out["rendered_answer"] == {"spoken": f"According to Wikipedia, {ANSWER}", "written": None}
    assert out["messages"][0].content == f"According to Wikipedia, {ANSWER}"
...
def test_call_main_model_message_keeps_the_tool_calls(fake_llm):
    from apu.inference import llm

    fake_llm.main_replies = [tool_call_reply("web_search", {"query": "fractions"})]
    message = llm.call_main_model_message([{"role": "user", "content": "q"}],
                                          tools=[web_search.WEB_SEARCH_TOOL])
    assert message.content is None
    assert message.tool_calls[0].function.name == "web_search"
    assert llm.call_main_model([{"role": "user", "content": "q"}]) is None, (
...
@pytest.mark.parametrize("output_channel,expected", [
    ("voice", "read aloud"), ("braille", "read in braille"), ("text", None),
])
