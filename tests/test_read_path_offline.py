...
async def test_the_read_path_reaches_generation_offline(
    ...
    assert len(fake_llm.main_calls) == 1, "the turn got all the way to generation"
...
async def test_generation_is_the_only_remaining_network_dependency(
    ...
    monkeypatch.setattr(mmu, "search_memory", spy_search)
    fake_llm.main_replies = [OSError("network is unreachable")]

    with pytest.raises(OSError):
        await agent.planner_node(_state())

    assert reached["search"] is True, "retrieval ran before the failure"
    assert len(fake_llm.main_calls) == 1, "the failure came from generation"
    assert fake_llm.extraction_calls == [], "no write-back for a failed turn"
...
async def test_everything_after_the_embedding_works_offline(
    ...
    fake_llm.main_replies = ["Great question. What do you already know about halves?"]
    fake_llm.extraction_replies = [json.dumps({
        "student_profile": "",
    ...
    assert len(fake_llm.main_calls) == 1
...
async def test_course_context_reaches_the_prompt(
    ...
    system_text = fake_llm.main_calls[0]["messages"][0]["content"]
...
async def test_the_system_prompt_is_sent_with_the_system_role(
    ...
    first = fake_llm.main_calls[0]["messages"][0]
...
async def test_the_window_reaches_the_model_with_openai_roles(
    ...
    sent = fake_llm.main_calls[0]["messages"][1:]      # [0] is the system prompt
