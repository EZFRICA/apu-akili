...
Ported from Akili (app_local/runtime/agent.py). What changed in the port, and why:
  ...
  - Inference goes through apu.inference.llm: call_main_model_message for the
  ...
  - Web search is a native OpenAI tool (Method A, see docs/decisions.md). It is offered only on
...
# How to write for each non-screen output. Without it the tutor answers in Markdown and
# LaTeX (observed live: \( \frac{1}{4} \)), which a speech synthesiser reads out symbol by
# symbol and which turns into meaningless cells in braille.
OUTPUT_INSTRUCTIONS = {
    OutputChannel.VOICE: """
OUTPUT: your answer will be read aloud. Write short spoken sentences in plain text: no Markdown, no LaTeX, no tables, no lists of symbols. Say formulas in words (for example "one quarter plus one sixth equals five twelfths").
""",
    OutputChannel.BRAILLE: """
OUTPUT: your answer will be read in braille. Write plain text only: no Markdown, no LaTeX, no emoji. Write formulas inline with ordinary characters (for example 1/4 + 1/6 = 5/12).
""",
}

# Found live when the tutor moved to a model that follows its sources: an English question
# about Ivorian exam dates was answered in French, because the pages found were French. The
# pupil's language is the pupil's, not the sources'.
LANGUAGE_INSTRUCTION = """
LANGUAGE: answer in the language the student wrote in, whatever language the course extracts, the search results or these instructions are in.
"""
...
def _llm():
    """
    The inference router, imported on first use rather than at import.

    It reads a provider key when a role is first called, so a device missing one key still
    starts and still runs the roles it can: a missing key fails a question, not the app.
    """
...
async def _call_extraction_model(prompt: str) -> str:
    ...
    # memories on different runs. response_format is OpenAI's JSON mode, verified live on
    # the configured provider. If it is rejected, the error is reported as a memory problem,
    # not swallowed.
    raw_extraction = await asyncio.to_thread(
        _llm().call_extraction_model,
...
async def _run_tool_call(call, tools: _TurnTools) -> str:
    ...
    # The second gate: the guard classified the student's message, and this classifies what
    # the model decided to go and fetch. A real lesson can carry an off-topic errand ("explain
    # averages using last night's match score"), which the first gate passes and this one
    # stops. Anything but a school verdict means no search, including a failed check.
    verdict = await classify_search_query(query)
    if verdict is not TurnOutcome.ON_TOPIC:
        logger.info("Search refused (%s): %r", verdict.value, query)
        tools.refused_searches.append(query)
        return (
            "This search is not school use, so it was not run. Answer from the course context "
            "and your own knowledge, and stay on the student's schoolwork."
        )
...
async def _run_notebook_save(call, tools: _TurnTools) -> str:
    ...
    except Exception as error:  # the model unreachable for the key points, store failure
...
async def _generate(state: AgentState, user_query: str, system_prompt: str,
    ...
    rendered = render_answer(answer_text, sources, mode)
