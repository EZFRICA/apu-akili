...
with tab_events:
    ...
    for event, resolution in sorted(events, key=lambda pair: pair[0].triggered_at, reverse=True):
        with st.container(border=True):
            ...
            st.markdown(f'<div class="apu-quote">{event.off_topic_request_text}</div>', unsafe_allow_html=True)
            if resolution:
    ...
    if snapshot and snapshot.clusters:
        ...
        for cluster in snapshot.clusters:
            with st.container(border=True):
                ...
                st.markdown(f'<div class="apu-quote">{cluster.representative_text}</div>', unsafe_allow_html=True)
