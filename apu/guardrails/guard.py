...
(TODO in docs/decisions.md): a policy is read when a session opens and applies to new sessions.
"""
...
class TopicalGuard:
    def __init__(
        ...
        *,
        ...
        # llm=None: NeMo builds the main model from config.yml (see config.GUARD_MODEL).
    ...
    async def check(self, session_id: str, user_text: str, exchange: str = "") -> GuardDecision:
        ...
        if outcome in (TurnOutcome.OFF_TOPIC, TurnOutcome.WELFARE):
