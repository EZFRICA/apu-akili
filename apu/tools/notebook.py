...
This is how a student who does not use the Save button (voice, braille) keeps something:
...
from apu.notebook.store import EntryKind, EntryOrigin, NotebookEntry, NotebookStore, new_entry
...
SAVE_TO_NOTEBOOK_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "to them and is printed in braille."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
def append_to_notebook(
    ...
    """Save an entry to the student's notebook from conversational/voice intents."""
    ...
    saved = notebook_store.add(entry)
    return {
        "entry_id": saved.entry_id,
        "student_id": saved.student_id,
        "title": title or saved.subject,
        "content": saved.text,
        "class_level": saved.class_level,
        "subject": saved.subject,
        "created_at": saved.created_at.isoformat(),
    }


def read_notebook(student_id: str, store: NotebookStore | None = None) -> dict:
    """Read entries from the student's notebook for summary or review."""
    notebook_store = store or NotebookStore()
    entries = notebook_store.entries(student_id=student_id)
    return {
        "student_id": student_id,
        "entries": [
            {
                "entry_id": e.entry_id,
                "title": e.subject or "Note",
                "content": e.text,
