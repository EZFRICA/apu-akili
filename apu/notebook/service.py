"""Saving to the notebook, and turning it into a revision sheet, on screen or in braille.

Two model calls, both made only on the student's request, and both on the small write-back
model (config.EXTRACTION_MODEL):
  - key points: one answer condensed at save time;
  - revision summary: a set of entries rewritten into one sheet.

Both were on the tutor model until the models were re-measured after the routing changed. The
reason they moved there in the first place was a verbose reasoner in the small role, which
spent 1700 tokens to return 140 characters. With the current small model the picture reverses:
0.81 s against 6.89 s for the key points, 0.88 s against 3.53 s for the sheet, with output at
least as well structured. Measure per configuration, not once (docs/models.md).
Both calls are asked for plain text, so an entry stays readable wherever it is shown,
including once translated to braille (apu.modality.braille.sheet).
"""
...
SUMMARY_PROMPT = """Write one revision sheet from these notes a student saved during their lessons. The student may read it in braille, so keep it short and well ordered: group related notes under a short title line, keep every definition, rule and method, drop repetitions.
...
async def _ask(call_name: str, prompt: str, temperature: float) -> str:
    ...
    for _ in range(2):
        ...
        if reply and reply.strip():
            # Models end lines with Markdown hard breaks ("  "), noise in a notebook.
            return "\n".join(line.rstrip() for line in reply.strip().splitlines())
    raise NotebookUnavailable("The model returned no text twice.")


async def condense_key_points(answer: str) -> str:
    prompt = KEY_POINTS_PROMPT.format(rules=PLAIN_TEXT_RULES, answer=answer)
    # The small model: 0.81 s against 6.89 s on the tutor model, for the same list.
...
async def summarize_entries(entries: Sequence[NotebookEntry]) -> str:
    ...
    return await _ask("call_extraction_model", prompt, temperature=0.3)
